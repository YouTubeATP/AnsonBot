  ...
  run: async (client, message, args, shared) => {
    ...
    if (!cases || cases.length == 0)
      return await message.channel.send(
        fn.embed(client, `${target} has no infractions yet!`)
      );
    cases = cases.filter(
      thisCase => thisCase.user == user.id && !thisCase.type.includes("UN")
    );
    if (!cases || cases.length == 0)
      return await message.channel.send(
        fn.embed(client, `${target} has no infractions yet!`)
    ...
    return await message.channel.send(
      new Discord.RichEmbed()
        ...
        .addField(
          "Last 7 Days",
          cases.filter(c => moment() < moment(c.time) + 1000 * 60 * 60 * 24 * 7)
            .length,
          true
        )
