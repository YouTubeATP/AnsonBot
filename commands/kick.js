...
module.exports = {
  ...
  run: async (client, message, args, shared) => {
    ...
    if (
      ...
      return message.channel.send(
        fn.embed(client, "You cannot kick a moderator!")
      );
    ...
    if (
      ...
      return message.channel.send(
        fn.embed(
          client,
          `You do not have permissions to kick ${target.user.username}!`
        )
      );
    if (!target.kickable)
      return message.channel.send(
        fn.embed(
          client,
          `I do not have permissions to kick ${target.user.username}!`
        )
      );
    ...
    target.user.send(
      fn.embed(client, `You have been kicked from ${message.guild.name}!`)
    );
    target.user
      .send(embed)
      .catch(error =>
      ...
      .then(() => {
        target
          ...
          .then(() => {
            ...
            console.log(
              `${message.guild.name} | Kicked ${target.user.tag} (${target.user.id})`
            );

            message.channel.send(
              fn.embed(client, `${target.user.tag} has been kicked!`)
            );
          ...
          .catch(error => {
            message.channel.send(
              fn.error(client, `I cannot kick ${target.user.tag}!`, error)
            );
          });
      });
