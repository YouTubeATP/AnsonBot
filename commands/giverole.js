...
module.exports = {
  ...
  description: "Gives roles to users.",
  category: "Moderation",
  aliases: ["addrole"],
  ...
  run: async (client, message, args, shared) => {
    if (!args[0])
      return message.channel.send(
        fn.embed(client, "Please mention the user you want to give a role to.")
      );
    ...
    if (!args[1])
      return message.channel.send(
        fn.embed(client, "Please mention the role you want to give.")
      );
    ...
    if (!message.guild.me.hasPermission("MANAGE_ROLES"))
      return message.channel.send(
        fn.embed(client, "I do not have permissions to give roles.")
      );
    ...
    target.roles
      ...
      .catch(error => {
        message.channel.send(
          fn.error(
            client,
            `I cannot give ${role.name} to ${target.user.tag}!`,
            error
          )
        );
      });
