  ...
  run: async (client, message, args, shared) => {
    if (!args[0])
      return message.channel.send(
        fn.embed(client, "Please mention the user you want to unban.")
      );
    ...
    message.guild
      ...
      .then(({ user }) => {
        ...
        message.guild.members
          ...
          .then(() => {
            let cases = [];
            if (modCases.has(message.guild.id))
              cases = modCases.get(message.guild.id);
            ...
            message.channel.send(
              fn.embed(
                client,
                `${user.tag} has been unbanned from ${message.guild.name}!`
              )
            );
          ...
          .catch(error => {
            message.channel.send(
              fn.error(client, `I cannot unban ${user.tag}!`, error)
            );
          });
      })
      .catch(error => {
